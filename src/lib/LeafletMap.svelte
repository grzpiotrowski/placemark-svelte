<script lang="ts">
	...
	export function addPopupMarkerAndZoom(layer: string, marker: MarkerSpec) {
		if (imap) {
			addPopup(layer, marker.name, marker.location);
	...
	export function populateLayer(markerLayer: MarkerLayer) {
		...
		markerLayer.markerSpecs.forEach((markerSpec) => {
			...
			const popupTitle = `<a href='/poi/${markerSpec.id}'>${markerSpec.name} <small>(click for details}</small></a>`;
		...
		addLayer(markerLayer.name, group);
		control.addOverlay(group, markerLayer.name);
	}
